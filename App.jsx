...
// pressure_msl = mean sea level pressure (CORRECT - matches Environment Canada)
// surface_pressure = elevation-adjusted station pressure (WRONG - reads ~996 hPa for Toronto at 76m elevation)
// inHg conversion: hPa * 0.02953
const hpaToInhg = v => v != null ? v * 0.02953 : null;
...
  // pressure stored internally as hPa, displayed as inHg
  // targets in hPa: 1008-1025, ideal 1016
  // in inHg: 29.78-30.27, ideal 30.01
  pressure: { min:1008,ideal:1016,max:1025, label:"Sea Level P",  unit:"inHg", key:"pressure_msl",               where:"sea level, Toronto (corrected)" },
};
...
    push(d.pressure, h.pressure_msl?.[i]); // CORRECTED: pressure_msl not surface_pressure
  });
  const result={};
...
      pressure:avg(arrs.pressure), // stored as hPa, converted to inHg for display
    };
  });
  return result;
}
...
    // pressure val is in hPa, targets are in hPa
...
  const g3=raw?.geo300?.toFixed(0),pr=fmtInhg(hpaToInhg(raw?.pressure)),w5=raw?.wind500?.toFixed(0);
  if(status==="perfect")return`Score ${composite}/100 — Pipe at working pressure over Toronto. 300 hPa jet ${w3} km/h at ~9km altitude, 200 hPa jet ${w2} km/h at ~12km, geopotential ${g3} m (normal ~9390 m), sea level pressure ${pr} inHg. Systems steering east cleanly. God holds the dial here.`;
  if(status==="high"){
    const r=[];
    if(raw?.wind300>185)r.push(`300 hPa jet over Toronto (${w3} km/h at 9km) kinked past 185 km/h target`);
...
    if(raw?.pressure<1008)r.push(`sea level pressure at Toronto (${pr} inHg) shows active cyclone compressing column`);
    if(raw?.pressure>1025)r.push(`sea level pressure (${pr} inHg) dominated by arctic high clamping jet from north`);
...
// Display value — pressure shown in inHg, everything else as-is
function displayVal(key,val){
  if(val==null)return"N/A";
  if(key==="pressure") return fmtInhg(hpaToInhg(val));
  return val.toFixed(key==="geo300"?0:1);
}

// Display unit
function displayUnit(key){ return TARGETS[key].unit; }

// Display diff — pressure shown in inHg diff
function displayDiff(key,diff){
  if(diff==null)return"—";
  const d=key==="pressure"?diff*0.02953:diff;
  return d>=0?`+${d.toFixed(key==="pressure"?3:1)}`:`${d.toFixed(key==="pressure"?3:1)}`;
}

// Target range display
function displayTargetRange(key){
  const t=TARGETS[key];
  if(key==="pressure") return`${fmtInhg(hpaToInhg(t.min))}–${fmtInhg(hpaToInhg(t.max))} inHg · ideal ${fmtInhg(hpaToInhg(t.ideal))} inHg`;
  return`${t.min}–${t.max} ${t.unit} · ideal ${t.ideal}`;
}
...
  return(
    <div style={{background:C.bg,height:"100vh",width:"100vw",color:C.text,fontFamily:"'Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",overflow:"hidden",boxSizing:"border-box",padding:"6px 10px"}}>
      <style>{`
...
          <span>{loading?"Fetching Open-Meteo…":error?`Error: ${error.slice(0,60)}`:"Live · ERA5+GFS · Toronto 43.7°N 79.4°W · pressure_msl corrected"}</span>
        </div>
      </div>
...
            {c:C.perfect,e:"🟣",t:"OPTIMAL — Working Pressure",d:"Jet 130–185 km/h, flat zonal flow, geo ≥9350 m, SLP 29.78–30.27 inHg."},
          ].map(({c,e,t,d})=>(
...
                <div style={{color:C.muted,fontFamily:"monospace",fontSize:"0.57rem"}}>{displayTargetRange(k)}</div>
              </div>
            ))}
...
      {/* MODAL */}
      {modalDay&&modalInfo&&(
        <div onClick={e=>{if(e.target===e.currentTarget)setModalDay(null);}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:12,backdropFilter:"blur(3px)"}}>
...
                    <div style={{fontSize:"0.57rem",fontFamily:"monospace",color:"#7c3aed",marginBottom:4}}>📍 {t.where}</div>
                    <div style={{fontSize:"1.2rem",fontWeight:900,fontFamily:"monospace",color,marginBottom:1}}>
                      {displayVal(k,val)}
                      <span style={{fontSize:"0.68rem",marginLeft:3}}>{displayUnit(k)}</span>
                    </div>
...
                      <span style={{color:diff==null?C.muted:diff>0?C.high:C.low}}>
                        {diff==null?"→ —":`${diff>0?"↑":"↓"} ${displayDiff(k,diff)} ${displayUnit(k)} vs prev day`}
                      </span>
                    </div>
                    <div style={{fontSize:"0.58rem",color:C.muted,fontFamily:"monospace",marginBottom:4}}>
                      Target {displayTargetRange(k)}
