...
// NOTE: Open-Meteo Historical Forecast API uses "pressure_msl" for sea-level pressure,
// not "surface_pressure" (which is station pressure, uncorrected for elevation).
const TARGETS = {
...
async function fetchMonthData(year, month) {
  ...
  // Build pressure-level variables list
  // The API uses e.g. &hourly=wind_speed_200hPa,wind_speed_300hPa,...
  // plus pressure_msl as a standard hourly variable
  const pressureVars = [
    "wind_speed_200hPa",
    "wind_speed_300hPa",
...
function parseHourly(json) {
  ...
  Object.entries(byDate).forEach(([date, arrs]) => {
    ...
    result[date] = {
      wind200:  max(arrs.wind200),   // daily peak jet speed
      wind300:  max(arrs.wind300),
      geo300:   avg(arrs.geo300),    // daily average geopotential
      wind500:  max(arrs.wind500),
      pressure: avg(arrs.pressure),  // daily average sea-level pressure
    };
  });
  return result;
}
...
function scoreMetric(key, val) {
  ...
  if (key === "geo300" || key === "pressure") {
    // Higher = better (ridge / high pressure = warmer, more stable)
    if (val < t.min) {
      // Below min: scale 0–50 over a 300-unit deficit range
      return Math.max(0, ((val - (t.min - 300)) / 300) * 50);
    }
    if (val > t.max) {
      // Above max: slight penalty (too strong a ridge = stagnation)
      return Math.max(0, 100 - ((val - t.max) / 100) * 20);
    }
    return 50 + ((val - t.min) / (t.max - t.min)) * 50;
  } else {
    // Wind: sweet spot scoring — too slow OR too fast both bad
...
function calcDay(raw) {
  ...
  // 55–75 = purple (perfect), >75 = red (too high/over-amplified), <55 = blue (too low)
...
function metricColor(key, val) {
  ...
  const t = TARGETS[key];
  if (val < t.min) return "#3b82f6";  // blue = too low
  if (val > t.max) return "#ef4444";  // red = too high
  return "#a855f7";                   // purple = in range
}
...
// Build a plain-English summary for the detail panel
function buildSummary(selInfo) {
  ...
  if (status === "high") {
    const parts = [];
    if (raw?.wind200 != null && raw.wind200 > 210)
      parts.push(`200 hPa jet (${raw.wind200.toFixed(0)} km/h) is above the 210 km/h aggressive threshold`);
    if (raw?.wind300 != null && raw.wind300 > 185)
      parts.push(`300 hPa jet (${raw.wind300.toFixed(0)} km/h) is above the 185 km/h cap`);
    if (raw?.geo300 != null && raw.geo300 < 9200)
      parts.push(`geopotential height (${raw.geo300.toFixed(0)} m) is well below the 9350 m normal — deep cold trough overhead`);
    if (raw?.pressure != null && raw.pressure < 1008)
      parts.push(`sea-level pressure (${raw.pressure.toFixed(1)} hPa) is low — active cyclone or trough at the surface`);
    const detail = parts.length ? ` ${parts.join("; ")}. ` : " ";
    return `Score ${score}/100 — Dial trending too high or over-amplified.${detail}God would reduce amplitude and flatten the Rossby wave pattern northward.`;
  }

  // status === "low"
  const parts = [];
  if (raw?.wind200 != null && raw.wind200 < 130)
    parts.push(`200 hPa jet (${raw.wind200.toFixed(0)} km/h) is below the 130 km/h threshold — no productive jet driving flow over Toronto`);
  if (raw?.wind300 != null && raw.wind300 < 130)
    parts.push(`300 hPa jet (${raw.wind300.toFixed(0)} km/h) also collapsed`);
  if (raw?.geo300 != null && raw.geo300 > 9450 && (raw?.wind200 ?? 0) < 130)
    parts.push(`geopotential (${raw.geo300.toFixed(0)} m) is high but jet absent — stagnant cut-off ridge`);
  if (raw?.pressure != null && raw.pressure < 1010)
    parts.push(`pressure still low (${raw.pressure.toFixed(1)} hPa) with no steering flow to clear it`);
  const detail = parts.length ? ` ${parts.join("; ")}. ` : " ";
  return `Score ${score}/100 — Dial too low or collapsed.${detail}God would turn the speed dial up to 140–150 km/h to flush the stagnant system out and restore progressive flow.`;
}
...
const S = {
  ...
  mCardVal: (color) => ({
    fontSize: "1.35rem", fontWeight: 900, fontFamily: "monospace", color, marginBottom: 3,
  }),
...
export default function App() {
  ...
  function getInfo(dateStr) {
    ...
    const delta = (cur?.composite != null && prev?.composite != null)
      ? cur.composite - prev.composite
      : null;
    // Calendar color: perfect = purple; otherwise use the composite status directly
    // (direction arrows are shown but don't override the color — color = absolute state)
    ...
    <div style={S.app}>
      ...
      <div style={S.legend}>
        ...
        {[
          {
            c: "#ef4444", emoji: "⬆️",
            title: "Red — Dial Too High / Over-Amplified",
            desc: "Jet accelerating past target, Rossby wave amplifying, geopotential dropping. Cold trough digging over Toronto.",
          },
          {
            c: "#3b82f6", emoji: "⬇️",
            title: "Blue — Dial Too Low / Collapsed",
            desc: "Jet collapsed below threshold, low pressure stalling. Stuck cold or stagnant pattern over Toronto.",
          },
          {
            c: "#a855f7", emoji: "🟣",
            title: "Purple — In Target Range",
            desc: "All dials near optimal. Jet 130–185 km/h, geo ≥9350 m, SLP ≥1015 hPa. Progressive west-east flow.",
          },
      ...
      <div style={S.metricsBox}>
        <div style={S.metricsTitle}>
          Data Sources — Open-Meteo Historical Forecast API (Free · No Key Required)
        </div>
        <div style={S.metricsGrid}>
          ...
          <div style={S.metricItem}>
            ...
            <div style={S.metricItemDesc}>
              Average of all 5 metrics. 55–75 = 🟣 purple. &lt;55 = 🔵 blue. &gt;75 = 🔴 red.
            </div>
          </div>
        </div>
      </div>
          ...
          {Array.from({ length: daysInMo }).map((_, i) => {
            ...
            return (
              <div
                ...
                onClick={() => {
                  if (!isFuture && info) setSelected(dateStr === selected ? null : dateStr);
                }}
              >
                ...
                {loading && !info && (
                  <div style={{
                    width: 12, height: 12,
                    border: "2px solid #1e2230", borderTopColor: "#a855f7",
          ...
          <div style={S.detail}>
            ...
            <div style={S.metricsCards}>
              {Object.entries(TARGETS).map(([k, t]) => {
                ...
                const diffArrow = diff == null ? "→"
                  : diff > 0.3 ? "↑" : diff < -0.3 ? "↓" : "→";
                const diffColor = diff == null ? "#6b7280"
                  : diff > 0.3 ? "#ef4444" : diff < -0.3 ? "#3b82f6" : "#6b7280";
                ...
                return (
                  <div key={k} style={S.mCard}>
                    ...
                    <div style={S.mCardVal(color)}>
                      ...
                      {" "}<span style={{ fontSize: "0.75rem" }}>{t.unit}</span>
                    </div>
                    <div style={S.mCardChange}>
                      <span style={{ color: diffColor }}>
                        {diffArrow} {diffStr} {t.unit} vs prev day
                      </span>
                    </div>
